...
int		ft_check_objects(char *obj_str)
{
	...
	if ((obj = ft_pick_str(obj_str, "plane {", "}")) != NULL)
		if (ft_check_plane(obj))
			return ft_error(NULL, 5);
	ft_strdel(&obj);
	if ((obj = ft_pick_str(obj_str, "cone {", "}")) != NULL)
		if (ft_check_cone(obj))
			return ft_error(NULL, 4);
	ft_strdel(&obj);
	if ((obj = ft_pick_str(obj_str, "cylinder {", "}")) != NULL)
		if (ft_check_cylinder(obj))
			return ft_error(NULL, 3);
	ft_strdel(&obj);
	if ((obj = ft_pick_str(obj_str, "sphere {", "}")) != NULL )
...
int		ft_obj_check(char *str_file)
{
	...
	if (ft_strstr(obj_str, "plane {") == NULL && ft_strstr(obj_str, "sphere {") == NULL
	&& ft_strstr(obj_str, "cylinder {") == NULL && ft_strstr(obj_str, "cone {") == NULL)
